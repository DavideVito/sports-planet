                ...
                <Card className={classes.root}>
                    <CardHeader
                        ...
                        title={<div><h2>{post.owner.displayName}</h2><h3>{post.titolo}</h3>
                            <h5>{post.dataPostato.toDate().toLocaleDateString()}</h5></div>}
                    />
                    <CardMedia>
                        <iframe style={{
                            width: "100%",
                    ...
                    <CardActions disableSpacing>
                        <div>
                            {like}
                            {isLiked ? (
                                <button className={"button"} style={{padding:"10px",marginLeft:"10px"}} onClick={togliLike}>Togli Like</button>
                            ) : (
                                <button className={"button"} style={{padding:"10px",marginLeft:"10px"}} onClick={mettiLike}><FavoriteIcon></FavoriteIcon> Metti Like</button>
                            )}
                        </div>
                    ...
                    <Collapse in={expanded} timeout="auto" unmountOnExit>
                        <CardContent>
                            ...
                            <br/><br/>
                            <button className={"button"} style={{padding:"10px",width:"100%"}}
                                onClick={() => {
                            ...
                            {mostraCommenti && (
                                <>
                                    <div>
                                        {commentData.data && <strong><p>Commenti</p></strong>}
                                        <div>
                                    ...
                                    <div>
                                        <button
                                            className={"button"} style={{padding:"10px"}}
                                            onClick={() => {
                                        ...
                                        {showAddCommento && (
                                            <form onSubmit={addCommento}>
                                                ...
                                                />
                                                <button className={"button"} style={{padding:"10px"}}>Aggiungi</button>
                                            </form>
                                        )}
                                    </div>
                                </>
